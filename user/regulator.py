...
def train(data_sample_train, setup):  # train NN
    ...
    simulator = ForwardEuler(model=MechanicalSystem(dt, setup.n_x, setup.hidden), dt=dt)
...
def test(params, U_test, setup, y=np.ones((2, 1)), ahead_step=0):  # test NN, non-aging
    ...
    x_fit = params[1]
    simulator = ForwardEuler(model=MechanicalSystem(setup.dt, setup.n_x, setup.hidden), dt=setup.dt)
...
def regulator(params, Y, U, setup):
    ...
    simulator = ForwardEulerPEM(model=MechanicalSystem(setup.dt, setup.n_x, setup.hidden), factor=factor, dt=setup.dt,
